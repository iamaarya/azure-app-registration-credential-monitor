...
if (
    ...
    $TestParameters.TargetApplicationAppIds =
...
if (
    ...
    $TestParameters.TargetApplicationDisplayNames =
        $TargetApplicationDisplayNames
}
...
$EscapedAppId = $TargetApplicationAppId -replace "'", "''"

if ([string]::IsNullOrWhiteSpace($TargetApplicationAppId)) {
    $Uri =
        "$BaseUri?`$select=id,appId,displayName,passwordCredentials,keyCredentials&`$top=999"
}
else {
    ...
    $Uri =
        ...
        "&`$top=999"
    Write-Output "Target Application (client) ID: $TargetApplicationAppId"
}
...
if (-not [string]::IsNullOrWhiteSpace($TargetApplicationAppId) -and
    $Applications.Count -eq 0) {
    throw "Target App Registration was not found: $TargetApplicationAppId"
}
